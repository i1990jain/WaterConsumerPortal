...
public class RegisterService {
	...
	public Map<String, String> registration(RegisterData registerdata)
		...
		try {
			...
			if (householdid != 0) {
				//if the smartMeter is being not being checked.Check for smartMeter and building Id before registration.

				if(registerdata.isSmartmeter()==false){
					...
					if (!smartMeterId.isEmpty()) {
						...
						if (buidlingID==0) {
							map.put("responsemsg","No buildingIDFound.Please click the Checkbox to Register as Non-Metered User");
						...
						map.put("responsemsg","NoSmartMeterIDFound.Please click the Checkbox to Register as Non-Metered User");
						map.put("response", "Unauthorized");
						checked=0;
					}	
				}else{
	...
	/*	void Insert(RegisterData registerdata){
		SessionFactory sessionFactory = HibernateUtil.getSessionFactory();
		...
		String selectoid = "SELECT oid FROM User ORDER BY oid DESC";
		System.out.println(selectoid);
		Query selectoidquery = session1.createQuery(selectoid);
		selectoidquery.setMaxResults(1);
		List result1 = selectoidquery.list();
		System.out.println(result1);
		System.out.println("resultset:" + result1);
